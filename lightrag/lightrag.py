...
class LightRAG(_RoleLLMMixin, _StorageMigrationMixin, _PipelineMixin):
    ...
    **Signature** — preserved unchanged from earlier LightRAG releases
    ...
    def _build_global_config(self) -> dict[str, Any]:
        ...
        # Живые ссылки на хранилища (asdict выше делает копии) — нужны для
        # подмешивания METAINFO и полного текста «целиком»-документов в контекст.
        # getattr с None: первый вызов _build_global_config() из __post_init__
        # происходит ДО инициализации хранилищ — тогда их просто нет.
        global_config["full_docs"] = getattr(self, "full_docs", None)
        global_config["doc_status"] = getattr(self, "doc_status", None)
    ...
    def __post_init__(self, addon_params: dict[str, Any] | None):
        ...
        if self.embedding_func is not None:
            wrapped_func = priority_limit_async_func_call(
                ...
                queue_name="Функция эмбеддингов",
    ...
    async def _process_extract_entities(
        ...
        try:
            ...
            error_msg = f"Не удалось извлечь сущности и связи: {str(e)}"
            logger.error(error_msg)
    ...
    async def _insert_done(
        ...
        log_message = "Данные из памяти сохранены на диск"
    ...
    async def aquery_data(
        ...
        # КРИТИЧНО для ACL отдела (#7): перенести ограничение по документам в
        # пересобранный data_param, иначе /query/data вернёт сущности/связи/
        # фрагменты ВСЕХ документов в обход прав отдела.
        data_param.allowed_file_paths = param.allowed_file_paths
        ...
        if query_result is None:
            no_result_message = "Запрос не вернул результатов"
            if data_param.mode == "naive":
                no_result_message = "Релевантные фрагменты документов не найдены."
        ...
        try:
            ...
            if param.mode in ["local", "global", "hybrid", "mix"]:
                ...
                if type(response) is str:
                    return {
                        "status": "success",
                        "message": "Ответ LLM в режиме bypass без потоковой передачи",
                        "data": {},
                        "metadata": {},
                        "llm_response": {
                            "content": response,
                    ...
                    return {
                        "status": "success",
                        "message": "Потоковый ответ LLM в режиме bypass",
                        "data": {},
                        "metadata": {},
                        "llm_response": {
                            "content": None,
    ...
    async def _purge_doc_chunks_and_kg(
        ...
        *,
        ...
        try:
            ...
            async with pipeline_status_lock:
                log_message = (
                    f"[очистка] {doc_id}: сущностей к перестроению: {len(entities_to_rebuild)}, "
                    f"к удалению: {len(entities_to_delete)}"
                )
                logger.info(log_message)
                pipeline_status["latest_message"] = log_message
                pipeline_status["history_messages"].append(log_message)
            ...
            async with pipeline_status_lock:
                log_message = (
                    f"[очистка] {doc_id}: связей к перестроению: {len(relationships_to_rebuild)}, "
                    f"к удалению: {len(relationships_to_delete)}"
                )
                logger.info(log_message)
                pipeline_status["latest_message"] = log_message
                pipeline_status["history_messages"].append(log_message)
        ...
        try:
            ...
            async with pipeline_status_lock:
                log_message = (
                    f"[очистка] {doc_id}: удалено фрагментов из хранилища: {len(chunk_ids)}"
                )
                logger.info(log_message)
                pipeline_status["latest_message"] = log_message
                pipeline_status["history_messages"].append(log_message)
        ...
        if relationships_to_delete:
            try:
                ...
                async with pipeline_status_lock:
                    log_message = (
                        f"[очистка] {doc_id}: удалено связей: "
                        f"{len(relationships_to_delete)}"
                    )
                    logger.info(log_message)
                    pipeline_status["latest_message"] = log_message
                    pipeline_status["history_messages"].append(log_message)
        ...
        if entities_to_delete:
            try:
                ...
                async with pipeline_status_lock:
                    log_message = (
                        f"[очистка] {doc_id}: удалено сущностей: "
                        f"{len(entities_to_delete)}"
                    )
                    logger.info(log_message)
                    pipeline_status["latest_message"] = log_message
                    pipeline_status["history_messages"].append(log_message)
    ...
    async def adelete_by_doc_id(
        ...
        **Concurrency Control Design:**
            ...
            if not pipeline_status.get("busy", False):
                ...
                pipeline_status.update(
                    {
                        ...
                        "latest_message": f"Начало удаления документа: {doc_id}",
                    }
                )
                # Initialize history messages
                pipeline_status["history_messages"][:] = [
                    f"Начало удаления документа: {doc_id}"
                ]
            else:
                ...
                if not job_name.startswith("deleting") or "document" not in job_name:
                    return DeletionResult(
                        status="not_allowed",
                        doc_id=doc_id,
                        message=f"Удаление запрещено: текущая задача '{pipeline_status.get('job_name')}' не является задачей удаления документов",
                        status_code=403,
                        file_path=None,
                    )
        ...
        async with pipeline_status_lock:
            log_message = f"Запуск процесса удаления документа {doc_id}"
            logger.info(log_message)
            pipeline_status["latest_message"] = log_message
            pipeline_status["history_messages"].append(log_message)
        ...
        try:
            ...
            if doc_status != DocStatus.PROCESSED:
                if doc_status == DocStatus.PENDING:
                    warning_msg = (
                        f"Удаление {doc_id} {file_path} (прежний статус: PENDING)"
                    )
                elif doc_status == DocStatus.PROCESSING:
                    warning_msg = (
                        f"Удаление {doc_id} {file_path} (прежний статус: PROCESSING)"
                    )
                elif doc_status == DocStatus.PREPROCESSED:
                    warning_msg = (
                        f"Удаление {doc_id} {file_path} (прежний статус: PREPROCESSED)"
                    )
                elif doc_status == DocStatus.FAILED:
                    warning_msg = (
                        f"Удаление {doc_id} {file_path} (прежний статус: FAILED)"
                    )
                else:
                    status_text = (
                        doc_status.value
                    ...
                    warning_msg = (
                        f"Удаление {doc_id} {file_path} (прежний статус: {status_text})"
                    )
            ...
            if not chunk_ids:
                ...
                if delete_llm_cache and metadata_cache_ids:
                    if not self.llm_response_cache:
                        no_cache_msg = (
                            f"Невозможно удалить кэш LLM для документа {doc_id}: "
                            "хранилище кэша недоступно"
                        )
                ...
                async with pipeline_status_lock:
                    log_message = (
                        f"Документ удалён без связанных фрагментов: {doc_id}"
                    )
                    logger.info(log_message)
                    pipeline_status["latest_message"] = log_message
                    pipeline_status["history_messages"].append(log_message)
            ...
            try:
                ...
                async with pipeline_status_lock:
                    log_message = f"Найдено затронутых сущностей: {len(entities_to_rebuild)}"
                    logger.info(log_message)
                    pipeline_status["latest_message"] = log_message
                    pipeline_status["history_messages"].append(log_message)
                ...
                async with pipeline_status_lock:
                    log_message = (
                        f"Найдено затронутых связей: {len(relationships_to_rebuild)}"
                    )
                    logger.info(log_message)
                    pipeline_status["latest_message"] = log_message
                    pipeline_status["history_messages"].append(log_message)
            ...
            if chunk_ids:
                try:
                    ...
                    async with pipeline_status_lock:
                        log_message = (
                            f"Успешно удалено фрагментов из хранилища: {len(chunk_ids)}"
                        )
                        logger.info(log_message)
                        pipeline_status["latest_message"] = log_message
                        pipeline_status["history_messages"].append(log_message)
            ...
            if relationships_to_delete:
                try:
                    ...
                    async with pipeline_status_lock:
                        log_message = f"Успешно удалено связей: {len(relationships_to_delete)}"
                        logger.info(log_message)
                        pipeline_status["latest_message"] = log_message
                        pipeline_status["history_messages"].append(log_message)
            ...
            if entities_to_delete:
                try:
                    ...
                    async with pipeline_status_lock:
                        log_message = (
                            f"Успешно удалено сущностей: {len(entities_to_delete)}"
                        )
                        logger.info(log_message)
                        pipeline_status["latest_message"] = log_message
                        pipeline_status["history_messages"].append(log_message)
            ...
            log_message = f"Документ {doc_id} успешно удалён"
            if delete_llm_cache and doc_llm_cache_ids:
                ...
                try:
                    ...
                    cache_log_message = f"Успешно удалено записей кэша LLM для документа {doc_id}: {len(doc_llm_cache_ids)}"
                    ...
                    log_message = (
                        f"Не удалось удалить кэш LLM для документа {doc_id}: "
            ...
            error_message = f"Ошибка при удалении документа {doc_id}: {e}"
            ...
            try:
                ...
                logger.error(traceback.format_exc())
                error_message = (
                    f"{error_message}. Кроме того, не удалось сохранить состояние для повтора: "
                    f"{status_update_error}. Может потребоваться ручная очистка."
                )
            return DeletionResult(
                status="fail",
                doc_id=doc_id,
                ...
                try:
                    ...
                    if deletion_fully_completed:
                        ...
                        return DeletionResult(
                            status="fail",
                            doc_id=doc_id,
                            message=f"Удаление завершено, но не удалось сохранить изменения: {persistence_error}",
                            status_code=500,
                            file_path=file_path,
                        )
            ...
            if we_acquired_pipeline:
                async with pipeline_status_lock:
                    ...
                    completion_msg = (
                        f"Процесс удаления документа завершён: {doc_id}"
                    )
