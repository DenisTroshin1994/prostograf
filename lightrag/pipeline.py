...
class _PipelineMixin:
    ...
    async def apipeline_enqueue_documents(
        ...
        async with pipeline_status_lock:
            if not from_scan and pipeline_status.get("scanning_exclusive"):
                raise RuntimeError(
                    "Невозможно добавить документы в очередь, пока сканирование "
                    "классифицирует файлы; дождитесь завершения фазы классификации "
                    "и повторите попытку."
                )
            if pipeline_status.get("destructive_busy"):
                raise RuntimeError(
                    "Невозможно добавить документы в очередь, пока пайплайн "
                    "очищает или удаляет документы; дождитесь завершения "
                    "текущей задачи и повторите попытку."
                )
            ...
            if duplicate_attempts:
                ...
                for index, attempt in enumerate(duplicate_attempts):
                    ...
                    if duplicate_kind == "content_hash":
                        error_prefix = (
                            "Идентичное содержимое уже существует под другим именем файла."
                        )
                    else:
                        error_prefix = "Файл с таким именем уже существует."
                    duplicate_docs[dup_record_id] = {
                        "status": DocStatus.FAILED,
                        "content_summary": (
                            f"[ДУБЛИКАТ:{duplicate_kind}] Исходный документ: "
                        ...
                        "error_msg": (
                            f"{error_prefix} "
                            f"Исходный doc_id: {attempt.get('original_doc_id', doc_id)}, "
                            f"статус: {attempt.get('existing_status', 'unknown')}"
                        ),
                        "metadata": {
                            "is_duplicate": True,
    ...
    async def apipeline_enqueue_error_documents(
        ...
        for error_file in error_files:
            ...
            error_description = error_file.get(
                "error_description", "Не удалось извлечь содержимое файла"
            )
            original_error = error_file.get("original_error", "Неизвестная ошибка")
    ...
    async def apipeline_process_enqueue_documents(self) -> None:
        ...
        try:
            ...
            while True:
                ...
                async with pipeline_status_lock:
                    if pipeline_status.get("cancellation_requested", False):
                        ...
                        if is_internal:
                            ...
                            log_message = f"Пайплайн отменён ({label})"
                ...
                if not to_process_docs:
                    log_message = "Все документы из очереди обработаны"
                    logger.info(log_message)
                    pipeline_status["latest_message"] = log_message
                    pipeline_status["history_messages"].append(log_message)
                ...
                if not to_process_docs:
                    log_message = (
                        "После проверки целостности не осталось документов для обработки"
                    )
                    logger.info(log_message)
                    pipeline_status["latest_message"] = log_message
                    pipeline_status["history_messages"].append(log_message)
                ...
                log_message = f"Обработка {len(to_process_docs)} документа(ов)"
                logger.info(log_message)
                ...
                log_message = "Обработка дополнительных документов по отложенному запросу"
                logger.info(log_message)
                pipeline_status["latest_message"] = log_message
                pipeline_status["history_messages"].append(log_message)
            ...
            stopped_message = "Пайплайн обработки документов из очереди остановлен"
    ...
    async def _validate_and_fix_document_consistency(
        ...
        if failed_docs_to_preserve:
            async with pipeline_status_lock:
                preserve_message = f"Сохранено записей сбойных документов для ручной проверки: {len(failed_docs_to_preserve)}"
        ...
        if inconsistent_docs:
            async with pipeline_status_lock:
                summary_message = (
                    f"Обнаружены несогласованные записи документов: {len(inconsistent_docs)}"
                )
            ...
            for doc_id in inconsistent_docs:
                try:
                    ...
                    async with pipeline_status_lock:
                        log_message = (
                            f"Удалена несогласованная запись: {doc_id} ({file_path})"
                        )
                        logger.info(log_message)
                        pipeline_status["latest_message"] = log_message
                        pipeline_status["history_messages"].append(log_message)
                    ...
                    async with pipeline_status_lock:
                        error_message = f"Не удалось удалить запись: {doc_id} - {str(e)}"
        ...
        if docs_to_reset:
            ...
            async with pipeline_status_lock:
                reset_message = (
                    f"Сброшено {reset_count} документов из статуса "
                    "PARSING/ANALYZING/PROCESSING/FAILED в PENDING"
                    + (
                        f"; нормализовано {normalized_count} документа(ов) PENDING "
                        "с устаревшими метаданными"
                        if normalized_count
                        else ""
                    )
                )
        ...
        while True:
            ...
            try:
                ...
                if not content_data_w:
                    raise Exception(
                        f"Содержимое документа не найдено в full_docs для doc_id: {doc_id_w}"
                    )
                ...
                async with ctx.pipeline_status_lock:
                    log_message = f"Парсинг ({engine}): {doc_id_w}"
                    logger.info(log_message)
                ...
                if doc_format_w == FULL_DOCS_FORMAT_PENDING_PARSE:
                    ...
                    if key_spec is not None and not key_spec.user_selectable:
                        raise ValueError(
                            f"внутренний парсер {key!r} не является допустимым "
                            f"движком для PENDING_PARSE: doc_id={doc_id_w}"
                        )
                ...
                if (
                    ...
                    if suffix_w not in suffix_capabilities(effective_key, specs):
                        raise ValueError(
                            f"движок {effective_key!r} не поддерживает "
                            f".{suffix_w or '<без расширения>'}: doc_id={doc_id_w}"
                        )
                parsed_data_w = (
                    await parser.parse(
        ...
        async with ctx.semaphore:
            try:
                ...
                async with ctx.pipeline_status_lock:
                    ...
                    log_message = (
                        f"Извлечение, этап {current_file_number}/"
                        f"{ctx.total_files}: {file_path}"
                    )
                    logger.info(log_message)
                    ctx.pipeline_status["history_messages"].append(log_message)
                    log_message = f"Обработка d-id: {doc_id}"
                    logger.info(log_message)
                    ctx.pipeline_status["latest_message"] = log_message
                    ctx.pipeline_status["history_messages"].append(log_message)
    ...
    async def _purge_stale_extraction_if_resuming(
        ...
        *,
        ...
        if intended_engine and stored_engine and intended_engine != stored_engine:
            log_message = (
                f"[возобновление] {doc_id}: подсказка в имени файла / "
                f"LIGHTRAG_PARSER предполагает движок "
                f"{intended_engine!r}, но в full_docs "
                f"уже задан parse_engine="
                f"{stored_engine!r}; сохраняем существующее "
                f"извлечение. Чтобы сменить движок, удалите "
                f"документ и загрузите его заново."
            )
        ...
        log_message = (
            f"[возобновление] {doc_id}: удаление "
            f"{len(stored_chunk_ids)} фрагмента(ов) и "
            f"связанных записей графа знаний от предыдущего запуска "
            f"перед перестроением с текущими "
            f"process_options"
        )
        logger.info(log_message)
        async with pipeline_status_lock:
    ...
    async def _raise_if_cancelled(
        ...
        async with pipeline_status_lock:
            if pipeline_status.get("cancellation_requested", False):
                raise PipelineCancelledException("Отменено пользователем")
    ...
    def _cancellation_label(pipeline_status: dict) -> str:
        ...
        if pipeline_status.get("cancellation_reason") == "internal_error":
            ...
            return f"Отменено из-за внутренней ошибки: {detail}"
    ...
    def _internal_halt_message(label: str) -> str:
        ...
        """
        return (
            f"Пайплайн остановлен из-за внутренней ошибки хранилища ({label}). "
            f"Устраните проблему с хранилищем и перезапустите обработку; "
            f"затронутые документы остаются в очереди (PENDING/FAILED)."
        )
    ...
    async def _mark_doc_cancelled_in_stage(
        ...
        *,
        ...
        """
        error_msg = (
            f"{self._cancellation_label(pipeline_status)} на этапе "
    ...
    async def _finalize_doc_failure(
        ...
        *,
        ...
        if isinstance(error, PipelineCancelledException):
            ...
            raw = str(error)
            if raw.startswith("Отменено пользователем"):
                doc_error_msg = f"{cancel_label}{raw[len('Отменено пользователем'):]}"
            elif raw:
            ...
            if stage_label == "merge":
                error_msg = (
                    f"{cancel_label} на этапе слияния {current_file_number}/"
            ...
            if stage_label == "merge":
                error_msg = (
                    f"Сбой на этапе слияния в документе "
                    f"{current_file_number}/{total_files}: {file_path}"
                )
            else:
                error_msg = (
                    f"Не удалось извлечь данные из документа "
                    f"{current_file_number}/{total_files}: {file_path}"
                )
    ...
    async def _mark_duplicate_after_parse(
        ...
        message = (
            "Идентичное содержимое уже существует под другим именем файла. "
            f"Исходный doc_id: {original_doc_id}, статус: {original_status}"
        )
        ...
        await self.doc_status.upsert(
            {
                doc_id: {
                    "status": DocStatus.FAILED,
                    "content_summary": (
                        f"[ДУБЛИКАТ:content_hash] Исходный документ: {original_doc_id}"
                    ),
        ...
        archive_msg = f"; перемещено в архив: {archived}" if archived else ""
        warning = f"Дубликат содержимого пропущен после парсинга: {file_path}{archive_msg}"
        ...
        try:
            ...
            def _required_json_string(
                ...
                if not isinstance(value, str) or not value.strip():
                    raise _MMJSONConformanceError(
                        f"{prefix}: отсутствует или некорректно поле '{field}'"
                    )
                return value.strip()
            ...
            async def _analyze_drawing(
                ...
                if not vlm_process_enable or use_vlm_func is None:
                    raise MultimodalAnalysisError(
                        f"drawings/{item_id}: требуется VLM-анализ, но "
                        "роль VLM недоступна "
                        "(VLM_PROCESS_ENABLE или настройка роли vlm)"
                    )
                try:
                    raw = candidate.read_bytes()
                except OSError as exc:
                    raise MultimodalAnalysisError(
                        f"drawings/{item_id}: не удалось прочитать изображение {candidate}: {exc}"
                    ) from exc
                if not raw:
                    raise MultimodalAnalysisError(
                        f"drawings/{item_id}: файл изображения пуст"
                    )
                ...
                async def _call_vlm_once() -> str:
                    try:
                        ...
                        raise MultimodalAnalysisError(
                            f"drawings/{item_id}: сбой вызова VLM: {exc}"
                        ) from exc
            ...
            async def _analyze_text_modality(
                ...
                if use_extract_func is None:
                    raise MultimodalAnalysisError(
                        f"{kind}/{item_id}: требуется роль EXTRACT, но она не настроена"
                    )
                ...
                if not content_text:
                    ...
                    raise MultimodalAnalysisError(
                        f"{kind}/{item_id}: отсутствует содержимое {kind}"
                    )
                ...
                if kind == "table":
                    ...
                    if fmt not in ("html", "json"):
                        raise MultimodalAnalysisError(
                            f"table/{item_id}: отсутствует или некорректен формат таблицы "
                ...
                if tokenizer is not None:
                    ...
                    if max_extract_tokens > 0 and total_tokens > max_extract_tokens:
                        ...
                        if content_budget <= 0:
                            ...
                            raise MultimodalAnalysisError(
                                f"{kind}/{item_id}: каркас промпта "
                                f"({frame_tokens} токенов) превышает "
                                f"MAX_EXTRACT_INPUT_TOKENS "
                                f"({max_extract_tokens}); увеличьте лимит"
                            )
                        ...
                        if final_tokens > max_extract_tokens:
                            raise MultimodalAnalysisError(
                                f"{kind}/{item_id}: усечённый промпт "
                                f"({final_tokens} токенов) всё ещё превышает "
                                f"MAX_EXTRACT_INPUT_TOKENS "
                ...
                async def _call_extract_once() -> str:
                    try:
                        ...
                        raise MultimodalAnalysisError(
                            f"{kind}/{item_id}: сбой вызова EXTRACT: {exc}"
                        ) from exc
            ...
            async def _run_with_progress_log(coro, kind: str, item_id: str):
                ...
                try:
                    ...
                    log_message = f"Анализ {kind}/{item_id}: сбой"
                    logger.warning(log_message)
                    if pipeline_status is not None and pipeline_status_lock is not None:
                        async with pipeline_status_lock:
                            pipeline_status["latest_message"] = log_message
                ...
                if is_success:
                    log_message = f"Анализ {kind}/{item_id}: успешно"
                    logger.info(log_message)
                    if pipeline_status is not None and pipeline_status_lock is not None:
                        async with pipeline_status_lock:
                            pipeline_status["latest_message"] = log_message
            ...
            for sidecar_path, root_key, kind, enabled in sidecars:
                ...
                try:
                    ...
                    raise MultimodalAnalysisError(
                        f"не удалось прочитать сайдкар-файл {sidecar_path}: {exc}"
                    ) from exc
                ...
                if (
                    ...
                    async with pipeline_status_lock:
                        log_message = f"Мультимодальный анализ: {doc_id}"
                        logger.info(log_message)
                        pipeline_status["latest_message"] = log_message
                        pipeline_status["history_messages"].append(log_message)
                    start_logged = True
                ...
                while pending:
                    if (
                        ...
                        fail_fast_exc = PipelineCancelledException(
                            "Отменено пользователем во время анализа"
                        )
                        break
            ...
            for local_idx, (item_id, item) in enumerate(items.items()):
                ...
                if status == "failure":
                    raise MultimodalAnalysisError(
                        ...
                        f"({analysis.get('message') or 'нет сообщения'})"
                    )
                if status != "success":
                ...
                if not name:
                    raise MultimodalAnalysisError(
                        f"{root_key}/{item_id}: в успешном результате отсутствует 'name'"
                    )
                if not description:
                    raise MultimodalAnalysisError(
                        f"{root_key}/{item_id}: в успешном результате отсутствует 'description'"
                    )
                if kind == "drawing" and not image_type:
                    raise MultimodalAnalysisError(
                        f"drawings/{item_id}: в успешном результате отсутствует 'type'"
                    )
                if kind == "equation" and not equation_body:
                    raise MultimodalAnalysisError(
                        f"equations/{item_id}: в успешном результате отсутствует 'equation'"
                    )
                ...
                if tokens > max_tokens:
                    ...
                    if tokens > max_tokens:
                        raise MultimodalAnalysisError(
                            f"{root_key}/{item_id}: мультимодальный фрагмент превышает "
                            f"{max_tokens} токенов даже после усечения описания "
                            f"до {min_desc_tokens} токенов"
                        )
