...
async def rebuild_knowledge_from_chunks(
    ...
    status_message = f"Перестроение знаний из {len(all_referenced_chunk_ids)} кэшированных извлечений фрагментов (параллельная обработка)"
    ...
    if not cached_results:
        status_message = "Кэшированные результаты извлечения не найдены, перестроение невозможно"
    ...
    for chunk_id, results in cached_results.items():
        try:
            ...
            status_message = (
                f"Не удалось разобрать кэшированный результат извлечения для фрагмента {chunk_id}: {e}"
            )
        ...
        async with semaphore:
            ...
            async with get_storage_keyed_lock(
                ...
                try:
                    ...
                    status_message = f"Не удалось перестроить `{entity_name}`: {e}"
                    logger.info(status_message)  # Per requirement, change to info
                    if pipeline_status is not None and pipeline_status_lock is not None:
                        async with pipeline_status_lock:
                            pipeline_status["latest_message"] = status_message
        ...
        async with semaphore:
            ...
            async with get_storage_keyed_lock(
                ...
                try:
                    ...
                    status_message = f"Не удалось перестроить `{src}`~`{tgt}`: {e}"
                    logger.info(status_message)  # Per requirement, change to info
                    if pipeline_status is not None and pipeline_status_lock is not None:
                        async with pipeline_status_lock:
                            pipeline_status["latest_message"] = status_message
    ...
    status_message = f"Запуск параллельного перестроения {len(entities_to_rebuild)} сущностей и {len(relationships_to_rebuild)} связей (параллельно: {graph_max_async})"
    logger.info(status_message)
    if pipeline_status is not None and pipeline_status_lock is not None:
        async with pipeline_status_lock:
            pipeline_status["latest_message"] = status_message
    ...
    # Final status report
    status_message = f"Перестроение графа знаний завершено: успешно перестроено {rebuilt_entities_count} сущностей и {rebuilt_relationships_count} связей."
    if failed_entities_count > 0 or failed_relationships_count > 0:
        status_message += f" Со сбоями: сущностей: {failed_entities_count}, связей: {failed_relationships_count}."
...
async def _rebuild_single_entity(
    ...
    status_message = f"Перестроение `{entity_name}` из {len(chunk_ids)} фрагментов"
    if truncation_info:
...
async def _rebuild_single_relationship(
    ...
    # Log rebuild completion with truncation info
    status_message = f"Перестроение `{src}`~`{tgt}` из {len(chunk_ids)} фрагментов"
    if truncation_info:
        status_message += f" ({truncation_info})"
...
async def _merge_nodes_then_upsert(
    ...
    try:
            ...
            async with pipeline_status_lock:
                if pipeline_status.get("cancellation_requested", False):
                    raise PipelineCancelledException(
                        "Отменено пользователем во время суммаризации сущности"
                    )
        ...
        if llm_was_used:
            status_message = f"LLM-слияние: `{entity_name}` | {already_fragment}+{num_fragment - already_fragment}"
        else:
            status_message = f"Слияние: `{entity_name}` | {already_fragment}+{num_fragment - already_fragment}"
    ...
    try:
        ...
        if pipeline_status is not None and pipeline_status_lock is not None:
            async with pipeline_status_lock:
                if pipeline_status.get("cancellation_requested", False):
                    raise PipelineCancelledException(
                        "Отменено пользователем во время суммаризации связи"
                    )
        ...
        already_fragment = len(already_description)
        if llm_was_used:
            status_message = f"LLM-слияние: `{src_id}`~`{tgt_id}` | {already_fragment}+{num_fragment - already_fragment}"
        else:
            status_message = f"Слияние: `{src_id}`~`{tgt_id}` | {already_fragment}+{num_fragment - already_fragment}"
        ...
        for need_insert_id in [src_id, tgt_id]:
            ...
            if existing_node is None:
                ...
                if updated:
                    status_message = (
                        f"Фрагменты добавлены из связи: `{need_insert_id}`"
                    )
                    logger.info(status_message)
                    if pipeline_status is not None and pipeline_status_lock is not None:
                        async with pipeline_status_lock:
        ...
        if relationships_vdb is not None:
            ...
            relation_status_message = f"Запись связи в векторную БД: `{relation_key}`"
...
async def merge_nodes_and_edges(
    ...
    if pipeline_status is not None and pipeline_status_lock is not None:
        async with pipeline_status_lock:
            if pipeline_status.get("cancellation_requested", False):
                raise PipelineCancelledException("Отменено пользователем во время фазы слияния")
    ...
    log_message = f"Слияние, этап {current_file_number}/{total_files}: {file_path}"
    ...
    log_message = f"Фаза 1: обработка {total_entities_count} сущностей из {doc_id} (параллельно: {graph_max_async})"
    logger.info(log_message)
    async with pipeline_status_lock:
        pipeline_status["latest_message"] = log_message
        pipeline_status["history_messages"].append(log_message)
        ...
        async with semaphore:
            ...
            if pipeline_status is not None and pipeline_status_lock is not None:
                async with pipeline_status_lock:
                    if pipeline_status.get("cancellation_requested", False):
                        raise PipelineCancelledException(
                            "Отменено пользователем во время слияния сущностей"
                        )
            ...
            async with get_storage_keyed_lock(
                ...
                try:
                    ...
                    error_msg = f"Ошибка обработки сущности `{entity_name}`: {e}"
                    logger.error(error_msg)
    ...
    log_message = f"Фаза 2: обработка {total_relations_count} связей из {doc_id} (параллельно: {graph_max_async})"
    logger.info(log_message)
    async with pipeline_status_lock:
        pipeline_status["latest_message"] = log_message
        pipeline_status["history_messages"].append(log_message)
        ...
        async with semaphore:
            ...
            if pipeline_status is not None and pipeline_status_lock is not None:
                async with pipeline_status_lock:
                    if pipeline_status.get("cancellation_requested", False):
                        raise PipelineCancelledException(
                            "Отменено пользователем во время слияния связей"
                        )
            ...
            async with get_storage_keyed_lock(
                ...
                try:
                    ...
                    error_msg = f"Ошибка обработки связи `{edge_label}`: {e}"
                    logger.error(error_msg)
    ...
    if full_entities_storage and full_relations_storage and doc_id:
        try:
            ...
            log_message = f"Фаза 3: итоговое обновление {len(final_entity_names)}({len(processed_entities)}+{len(all_added_entities)}) сущностей и {len(final_relation_pairs)} связей из {doc_id}"
            logger.info(log_message)
            async with pipeline_status_lock:
                pipeline_status["latest_message"] = log_message
                pipeline_status["history_messages"].append(log_message)
    ...
    log_message = f"Слияние завершено: сущностей: {len(processed_entities)}, дополнительных сущностей: {len(all_added_entities)}, связей: {len(processed_edges)}"
    logger.info(log_message)
    async with pipeline_status_lock:
        pipeline_status["latest_message"] = log_message
        pipeline_status["history_messages"].append(log_message)
...
async def extract_entities(
    ...
    if pipeline_status is not None and pipeline_status_lock is not None:
        async with pipeline_status_lock:
            if pipeline_status.get("cancellation_requested", False):
                raise PipelineCancelledException(
                    "Отменено пользователем во время извлечения сущностей"
                )
    ...
    async def _process_single_content(chunk_key_dp: tuple[str, TextChunkSchema]):
        ...
        log_message = f"Фрагмент {processed_chunks} из {total_chunks}: извлечено {entities_count} сущн. + {relations_count} связ. {chunk_key}"
        logger.info(log_message)
        ...
        async with semaphore:
            ...
            if pipeline_status is not None and pipeline_status_lock is not None:
                async with pipeline_status_lock:
                    if pipeline_status.get("cancellation_requested", False):
                        raise PipelineCancelledException(
                            "Отменено пользователем во время обработки фрагментов"
                        )
    ...
    if first_exception is not None:
        ...
        progress_prefix = f"Фрагмент[{processed_chunks + 1}/{total_chunks}]"
...
async def kg_query(
    ...
    # METAINFO + «целиком»: подмешиваем доп. контекст для найденных документов
    # (поверх собранного контекста — бюджет расширяется под полные документы).
    _supplement = await _build_doc_supplement(
        _present_file_paths(context_result.raw_data), query_param, global_config
    )
    if _supplement:
        context_result.context = (context_result.context or "") + _supplement
    ...
    # Сохраняем реальное число токенов промпта генерации (доминирующий член
    # стоимости запроса), чтобы вернуть его клиенту для точного учёта токенов.
    if isinstance(context_result.raw_data, dict):
        context_result.raw_data.setdefault("metadata", {})["llm_prompt_tokens"] = (
            len_of_prompts
        )
    ...
    args_hash = compute_args_hash(
        ...
        # ACL отдела в ключе кэша: иначе ответ, сгенерированный по документам
        # одного отдела, утёк бы пользователю другого отдела на тот же запрос.
        _acl_cache_marker(getattr(query_param, "allowed_file_paths", None)),
    )
...
def _acl_filter_items(items: list[dict], allowed: set[str] | None) -> list[dict]:
    """Фильтр доступа к документам (ACL отдела).

    Оставляет только элементы (чанки/сущности/связи), ВСЕ источники которых
    (`file_path`, возможно объединённые через GRAPH_FIELD_SEP) входят в
    `allowed`. `allowed=None` — без ограничений. Элемент без провенанса
    отбрасывается при активном ACL. Консервативно: если хоть один источник
    запрещён — элемент исключается целиком, чтобы исключить утечку через
    смешанные описания. Применяется ДО сборки контекста и списка источников,
    поэтому запрещённые документы физически не доходят до генератора."""
    if allowed is None:
        return items
    kept: list[dict] = []
    for it in items:
        ...
        if not fp:
            continue
        parts = [p.strip() for p in str(fp).split(GRAPH_FIELD_SEP) if p.strip()]
        if parts and all(p in allowed for p in parts):
            kept.append(it)
    return kept


def _acl_cache_marker(allowed: set[str] | None) -> str:
    """Стабильный отпечаток ACL отдела для ключа кэша ответов LLM.

    Кэш ответов (llm_response_cache) — общий на весь воркспейс. Без этого
    маркера один и тот же запрос от пользователей РАЗНЫХ отделов давал бы
    одинаковый ключ кэша, и пользователю одного отдела мог вернуться
    закэшированный ТЕКСТ ответа, сгенерированный по документам другого отдела
    (межотдельная утечка тела ответа в обход #7). None (admin/полный доступ) и
    пустое множество дают РАЗНЫЕ маркеры."""
    if allowed is None:
        return "ACL:ALL"
    return "ACL:" + "|".join(sorted(allowed))


def _present_file_paths(raw_data) -> set:
    """file_path'ы документов, реально попавших в результат (из references)."""
    out: set = set()
    try:
        for r in (raw_data or {}).get("data", {}).get("references", []) or []:
            fp = r.get("file_path")
            if fp:
                out.add(fp)
    except Exception:
...
async def _build_doc_supplement(present_files: set, query_param, global_config) -> str:
    """Доп. блок контекста для найденных документов (не меняет сами документы):

    - METAINFO (`query_param.doc_metainfo`): текст-приписка к документу;
    - «целиком» (`query_param.full_doc_paths`): полный текст документа, если он
      попал в результаты поиска (бюджет токенов при этом расширяется — блок
      добавляется поверх уже собранного контекста).
    """
    metainfo_map = getattr(query_param, "doc_metainfo", None) or {}
    full_set = getattr(query_param, "full_doc_paths", None) or set()
    if not present_files or (not metainfo_map and not full_set):
        return ""
    # Защита критического пути ACL (#7): этот блок строится только из документов,
    # уже прошедших фильтрацию и попавших в контекст. Если ACL активен — жёстко
    # пересекаем present_files с разрешёнными, чтобы никакая будущая правка не
    # смогла протащить сюда METAINFO/полный текст запрещённого отделу документа.
    allowed = getattr(query_param, "allowed_file_paths", None)
    if allowed is not None:
        present_files = {fp for fp in present_files if fp in allowed}
        if not present_files:
            return ""
    parts: list[str] = []
    for fp in sorted(present_files):
        mi = metainfo_map.get(fp)
        if mi:
            parts.append(f"[Дополнительные сведения о документе «{fp}»]\n{mi}")
    if full_set:
        full_docs = global_config.get("full_docs")
        doc_status = global_config.get("doc_status")
        if full_docs is not None and doc_status is not None:
            for fp in sorted(present_files):
                if fp not in full_set:
                    continue
                try:
                    res = await doc_status.get_doc_by_file_basename(fp)
                    if not res:
                        continue
                    doc_id, _ = res
                    full = await full_docs.get_by_id(doc_id)
                    content = (full or {}).get("content", "") if isinstance(full, dict) else ""
                    if content:
                        parts.append(
                            f"[Полный текст документа «{fp}» — приведён целиком, "
                            f"учитывай все шаги по порядку]\n{content}"
                        )
                except Exception as e:
                    logger.warning(
                        f"[doc supplement] не удалось получить полный текст {fp}: {e}"
                    )
    if not parts:
        return ""
    return "\n\n=== Дополнительный контекст документов ===\n" + "\n\n".join(parts)
...
async def _build_query_context(
    ...
    # ── ACL: ограничение по документам отдела (до любой сборки контекста) ──
    allowed_fp = getattr(query_param, "allowed_file_paths", None)
    if allowed_fp is not None:
        search_result["final_entities"] = _acl_filter_items(
            search_result["final_entities"], allowed_fp
        )
        search_result["final_relations"] = _acl_filter_items(
            search_result["final_relations"], allowed_fp
        )
        search_result["vector_chunks"] = _acl_filter_items(
            search_result["vector_chunks"], allowed_fp
        )
        kept_ids = {
            c.get("chunk_id") or c.get("id") for c in search_result["vector_chunks"]
        }
        search_result["chunk_tracking"] = {
            cid: meta
            for cid, meta in search_result["chunk_tracking"].items()
            if cid in kept_ids
        }
    ...
    # ACL (защита в глубину): отсечь любые чанки запрещённых документов,
    # подтянутые через связанные сущности/связи.
    if allowed_fp is not None:
        merged_chunks = _acl_filter_items(merged_chunks, allowed_fp)
...
async def naive_query(
    ...
    # ── ACL: ограничение по документам отдела ──
    allowed_fp = getattr(query_param, "allowed_file_paths", None)
    if allowed_fp is not None:
        chunks = _acl_filter_items(chunks, allowed_fp)
        if not chunks:
            logger.info("[naive_query] ACL: нет доступных отделу документов")
            return None
    ...
    # METAINFO + «целиком» для найденных документов (см. _build_doc_supplement).
    _supplement = await _build_doc_supplement(
        _present_file_paths(raw_data), query_param, global_config
    )
    if _supplement:
        context_content = context_content + _supplement
    ...
    # Реальное число токенов финального промпта (для точного учёта токенов
    # в ответе клиента).
    raw_data.setdefault("metadata", {})["llm_prompt_tokens"] = len(
    ...
    args_hash = compute_args_hash(
        ...
        "\n<llm_identity>\n",
        serialize_llm_cache_identity(llm_cache_identity),
        # ACL отдела в ключе кэша (см. _acl_cache_marker) — против межотдельной
        # утечки тела ответа через общий кэш.
        _acl_cache_marker(getattr(query_param, "allowed_file_paths", None)),
    )
    cached_result = await handle_cache(
        hashing_kv, args_hash, user_query, query_param.mode, cache_type="query"
