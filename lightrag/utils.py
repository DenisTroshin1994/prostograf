...
async def safe_vdb_operation_with_exception(
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            elapsed = time.perf_counter() - start_ts
            timeout_msg = (
                f"Тайм-аут операции VDB {operation_name} для {entity_name or '<неизвестно>'} "
                f"спустя {elapsed:.2f}с (попытка {attempt_label}, тайм-аут={timeout_seconds}с)"
            )
            if attempt >= max_retries - 1:
                log_func(timeout_msg)
                raise TimeoutError(timeout_msg) from e
            log_func(f"{timeout_msg}, повтор...")
            if retry_delay > 0:
            ...
            if attempt >= max_retries - 1:
                error_msg = (
                    f"Операция VDB {operation_name} не удалась для {entity_name or '<неизвестно>'} "
                    f"после {max_retries} попыток за {elapsed:.2f}с: {e}"
                )
                log_func(error_msg)
...
class WorkerTimeoutError(Exception):
    ...
    def __init__(self, timeout_value: float, timeout_type: str = "execution"):
        ...
        super().__init__(
            f"Тайм-аут воркера ({timeout_type}): превышен лимит {timeout_value} с"
        )
...
class HealthCheckTimeoutError(Exception):
    ...
    def __init__(self, timeout_value: float, execution_duration: float):
        ...
        super().__init__(
            f"Задача принудительно завершена из-за превышения времени выполнения "
            f"(>{timeout_value} с, фактически: {execution_duration:.1f} с)"
        )
            ...
            try:
                ...
                try:
                    ...
                    raise QueueFullError(
                        f"{queue_name}: очередь переполнена, тайм-аут {_queue_timeout} с"
                    )
                except Exception as e:
                ...
                try:
                    ...
                    cancelled_total += 1
                    raise TimeoutError(
                        f"{queue_name}: тайм-аут ожидания результата, {_timeout} с"
                    )
...
async def use_llm_func_with_cache(
    ...
    try:
        ...
        # Add [Вызов LLM] prefix to error message
        error_msg = f"[Вызов LLM] {str(e)}"
