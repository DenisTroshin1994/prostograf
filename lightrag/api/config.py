...
def parse_args() -> argparse.Namespace:
    ...
    # Настройки моделей, сохранённые из WebUI, перекрывают .env (без пересборки)
    from .user_llm_settings import apply_user_llm_overrides

    apply_user_llm_overrides()

    parser = argparse.ArgumentParser(description="ПростоГраф API Server")
    ...
    # ВАЖНО (безопасность/ACL #7): НЕ включаем сюда "/api/*". В апстриме Ollama-
    # совместимые роуты (/api/chat, /api/generate) были в белом списке и работали
    # БЕЗ аутентификации — то есть аноним мог через /api/chat выполнять RAG-поиск
    # по ВСЕМ документам в обход прав отдела. В многопользовательском режиме вход
    # обязателен, поэтому в белом списке остаётся только /health (его дёргает
    # docker healthcheck без токена).
