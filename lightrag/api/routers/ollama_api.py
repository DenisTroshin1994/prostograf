...
class OllamaAPI:
    ...
    def setup_routes(self):
        ...
        @self.router.get(
            ...
            summary="Версия (эмуляция Ollama)",
        )
        async def get_version():
            """Версия эмулируемого сервера Ollama"""
            return OllamaVersionResponse(version="0.9.3")

        @self.router.get(
            "/tags",
            dependencies=[Depends(combined_auth)],
            summary="Доступные модели (эмуляция Ollama)",
        )
        async def get_tags():
            """Список доступных моделей (эмуляция сервера Ollama)"""
        ...
        @self.router.get(
            "/ps",
            dependencies=[Depends(combined_auth)],
            summary="Запущенные модели (эмуляция Ollama)",
        )
        async def get_running_models():
            """Список запущенных в данный момент моделей"""
        ...
        @self.router.post(
            ...
            summary="Генерация (эмуляция Ollama)",
        )
        async def generate(raw_request: Request):
            """Обработка запросов генерации в режиме эмуляции модели Ollama.

            Для совместимости запрос не обрабатывается ПростоГраф,
            а передаётся напрямую базовой LLM-модели.
            Поддерживаются Content-Type application/json и application/octet-stream.
            """
            try:
        ...
        @self.router.post(
            ...
            dependencies=[Depends(combined_auth)],
            include_in_schema=True,
            summary="Чат (эмуляция Ollama)",
        )
        async def chat(
            raw_request: Request,
            user: Optional[Dict] = Depends(get_optional_user),
        ):
            """Обработка чат-запросов в режиме эмуляции модели Ollama.

            Направляет запросы пользователя через ПростоГраф, выбирая режим поиска по префиксу запроса.
            Служебные запросы OpenWebUI (генерация метаданных сессии) распознаются и передаются напрямую LLM.
            Поддерживаются Content-Type application/json и application/octet-stream.
            """
            try:
                # Parse the request body manually
                ...
                # ACL отдела (#7): ограничить документы по пользователю токена.
                # None — полный доступ (admin или доступ по API-ключу). Без этого
                # Ollama-совместимый чат обходил бы права отдела на документы.
                query_param.allowed_file_paths = resolve_allowed_file_paths_for_user(
                    user
                )
