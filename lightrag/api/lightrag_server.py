...
from lightrag.api.routers.user_settings_routes import create_user_settings_routes
...
# Защита /login от перебора — в отдельном модуле, чтобы admin-роуты могли
# сбрасывать блокировку при создании/смене/удалении пользователя.
...
def _inject_swagger_theme(html: str, theme: str) -> str:
    ...
    theme_snippet = textwrap.dedent(
        ...
        <style>
          ...
          /* Читаемость markdown-описаний: убираем «слипание» строк с
             инлайн-кодом — Swagger рисует пилюли кода без отступов, и при
             плотном межстрочном интервале они наезжают на соседние строки. */
          .swagger-ui .markdown p,
          .swagger-ui .markdown li,
          .swagger-ui .renderedMarkdown p,
          .swagger-ui .renderedMarkdown li,
          ...
          /* Воздух между операциями и скругление в стиле приложения */
          .swagger-ui .opblock {{
            margin-bottom: 14px;
            border-radius: 10px;
          }}
          ...
          html[data-lightrag-docs-dark="1"] .swagger-ui .opblock .opblock-section-header {{
            background: #2d2c29;
            color: #e8e6dc;
            box-shadow: none;
          }}

          html[data-lightrag-docs-dark="1"] .swagger-ui .opblock .opblock-section-header h4,
          html[data-lightrag-docs-dark="1"] .swagger-ui .opblock .opblock-section-header label {{
            color: #e8e6dc;
          }}

          /* Кнопки Try it out / Execute — в акцентный оранжевый */
          html[data-lightrag-docs-dark="1"] .swagger-ui .btn {{
            color: #e8e6dc;
            border-color: #3f3e3a;
          }}

          html[data-lightrag-docs-dark="1"] .swagger-ui .btn.execute {{
            background: #d97757;
            border-color: #d97757;
            color: #fdfcfa;
          }}

          html[data-lightrag-docs-dark="1"] .swagger-ui .btn.try-out__btn {{
            border-color: #d97757;
            color: #d97757;
            background: transparent;
          }}

          html[data-lightrag-docs-dark="1"] .swagger-ui .opblock .opblock-summary-path,
          html[data-lightrag-docs-dark="1"] .swagger-ui .opblock .opblock-summary-path a,
          html[data-lightrag-docs-dark="1"] .swagger-ui .opblock .opblock-summary-operation-id {{
            color: #e8e6dc;
          }}

          html[data-lightrag-docs-dark="1"] .swagger-ui .info a,
          html[data-lightrag-docs-dark="1"] .swagger-ui .markdown a,
          html[data-lightrag-docs-dark="1"] .swagger-ui .renderedMarkdown a {{
            color: #d97757;
          }}
        </style>
...
def create_app(args):
    ...
    async def lifespan(app: FastAPI):
        ...
        try:
            ...
            # Многопользовательский режим: создать стартового администратора
            # (admin/admin при первом запуске) и включить обязательный вход.
            from lightrag.api.repos.users_db import bootstrap_admin
            from lightrag.api.repos.paths import get_or_create_jwt_secret
            from lightrag.api.config import DEFAULT_TOKEN_SECRET
            import lightrag.api.utils_api as _utils_api

            bootstrap_admin()
            _utils_api.auth_configured = True

            # Безопасность: вход проверяется по users.db, а не по AUTH_ACCOUNTS,
            # поэтому штатная проверка TOKEN_SECRET не срабатывает и auth_handler
            # по умолчанию падает на публично известный DEFAULT_TOKEN_SECRET —
            # с ним любой может подделать admin-токен и обойти вход и ACL. Если
            # TOKEN_SECRET не задан явно (или равен дефолтному), подставляем
            # стойкий персистентный секрет из тома данных.
            if not auth_handler.secret or auth_handler.secret == DEFAULT_TOKEN_SECRET:
                auth_handler.secret = get_or_create_jwt_secret()
                logger.warning(
                    "TOKEN_SECRET не задан — используется автогенерируемый "
                    "персистентный секрет JWT из тома данных "
                    "(data/rag_storage/jwt_secret.txt). Подделка токенов исключена."
                )

            # Предупреждение, если администратор всё ещё с паролем по умолчанию.
            from lightrag.api.repos.users_db import authenticate as _authenticate

            _default_login = (
                os.getenv("RAG_BOOTSTRAP_ADMIN_LOGIN") or "admin"
            ).strip() or "admin"
            # Сверяем с ЭФФЕКТИВНЫМ паролем бутстрапа: если оператор задал
            # RAG_BOOTSTRAP_ADMIN_PASSWORD, предупреждаем при использовании именно
            # его (а не только литерала 'admin').
            _default_pw = (os.getenv("RAG_BOOTSTRAP_ADMIN_PASSWORD") or "admin") or "admin"
            _is_literal_admin = _default_pw == "admin"
            if _authenticate(_default_login, _default_pw):
                _pw_desc = (
                    "ПАРОЛЬ ПО УМОЛЧАНИЮ 'admin'"
                    if _is_literal_admin
                    else "неизменённый пароль бутстрапа из RAG_BOOTSTRAP_ADMIN_PASSWORD"
                )
                ASCIIColors.red(
                    f"\n⚠  ВНИМАНИЕ: администратор '{_default_login}' использует {_pw_desc}.\n"
                    f"    Смените его во вкладке «Отделы и пользователи» — пароль "
                    f"бутстрапа известен из конфигурации и небезопасен.\n"
                )
                logger.warning(
                    "Администратор '%s' использует %s — смените его в панели.",
                    _default_login,
                    _pw_desc,
                )
    ...
    base_description = (
        "API ядра ПростоГраф, веб-интерфейса и эмуляции моделей Ollama"
    )
    swagger_description = (
        base_description
        + (" (включён API-ключ)" if api_key else "")
        + "\n\n[Документация в формате ReDoc](/redoc)"
    )
    ...
    try:
        ...
        logger.error(f"Не удалось инициализировать ядро системы: {e}")
        raise
    ...
    from lightrag.api.routers.me_routes import create_me_routes
    from lightrag.api.routers.chat_routes import create_chat_routes

    # Управление документами/графом/настройками моделей и админка — только admin.
    # Запросы (/query*) доступны всем вошедшим, но с ACL по отделу (см.
    # query_routes). /me/* — для любого вошедшего пользователя.
    app.include_router(
        create_document_routes(rag, doc_manager, api_key),
        dependencies=[Depends(require_admin)],
    )
    app.include_router(create_query_routes(rag, api_key, args.top_k))
    app.include_router(
        create_graph_routes(rag, api_key), dependencies=[Depends(require_admin)]
    )
    app.include_router(
        create_user_settings_routes(api_key), dependencies=[Depends(require_admin)]
    )
    app.include_router(create_admin_routes(), dependencies=[Depends(require_admin)])
    app.include_router(create_me_routes(rag), dependencies=[Depends(get_current_user)])
    app.include_router(create_chat_routes(rag), dependencies=[Depends(get_current_user)])
    ...
    async def custom_swagger_ui_html(request: Request):
        ...
        html = _inject_swagger_theme(
            html, request.query_params.get("theme", "dark").lower()
        )
    ...
    @app.get("/", summary="Перенаправление в веб-интерфейс")
    ...
    @app.get("/auth-status", summary="Статус аутентификации")
    async def get_auth_status():
        """Статус аутентификации. Вход обязателен (многопользовательский режим);
        гостевой доступ отключён."""
        from lightrag.api.branding import get_branding
        ...
        return {
            ...
            "login_description": brand["login_description"],
        }

    @app.post("/login", summary="Вход в систему")
    async def login(request: Request, form_data: OAuth2PasswordRequestForm = Depends()):
        """Аутентификация по базе пользователей (users.db). Токен несёт роль
        и отдел; по роли WebUI решает: admin → приложение, user → /chat.

        Защита от перебора: после серии неудачных попыток вход по паре
        (IP, логин) временно блокируется."""
        from lightrag.api.repos.users_db import authenticate

        ip = login_throttle.client_ip(request)
        throttle_key = login_throttle.make_key(ip, form_data.username)
        if login_throttle.is_locked(throttle_key):
            logger.warning(
                "Вход временно заблокирован (перебор): ip=%s login=%s", ip, form_data.username
            )
            raise HTTPException(
                status_code=429,
        ...
        if not user:
            ...
            logger.warning("Неудачный вход: ip=%s login=%s", ip, form_data.username)
    ...
    @app.get("/me", summary="Текущий пользователь")
    ...
    @app.get(
        ...
        summary="Состояние системы и конфигурация",
        description="Возвращает полный статус системы: доступность веб-интерфейса, конфигурацию и операционные метрики",
        response_description="Статус системы с деталями конфигурации",
        responses={
            200: {
                "description": "Успешный ответ со статусом системы",
                "content": {
                    "application/json": {
                        "example": {
                            "status": "healthy",
    ...
    async def get_status(
        ...
        """Текущий статус системы, включая доступность веб-интерфейса.

        Анонимный вызов (без токена, например docker healthcheck) получает
        только безопасный минимум. Полная конфигурация (пути ФС, хосты, модели,
        хранилища, role_llm_config) — лишь для вошедших пользователей, чтобы не
        раскрывать инфраструктуру неаутентифицированным клиентам."""
        try:
            ...
            # Источник истины — utils_api.auth_configured, который выставляет
            # lifespan (модульная копия здесь не обновляется и всегда False).
            from lightrag.api import utils_api as _utils_api

            if not getattr(_utils_api, "auth_configured", auth_configured):
                auth_mode = "disabled"
            else:
                auth_mode = "enabled"
            ...
            # Анонимный вызов: только безопасный минимум, без раскрытия
            # инфраструктуры (путей ФС, хостов, моделей, хранилищ, конфигурации).
            if user is None:
                return {
                    "status": "healthy",
...
def configure_logging():
    ...
    print(f"\nФайл журнала сервера: {log_file_path}\n")
