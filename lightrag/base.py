...
class QueryParam:
    ...
    """Document-level access control (per-department ACL).

    None means unrestricted access (e.g. admin). A set restricts retrieval to
    the listed document file paths: any retrieved chunk, entity, or relation
    whose file_path is not within this set is dropped BEFORE it can reach the
    LLM context or the reference list. An empty set therefore yields no
    results at all. Set server-side from the authenticated user's department;
    ...
    doc_metainfo: dict[str, str] | None = None
    """Per-document METAINFO (file_path -> текст). Подмешивается в контекст
    генератора для документов, попавших в результаты поиска; сам документ не
    меняется. Ставится на сервере из doc_meta; не принимается от клиента."""

    full_doc_paths: set[str] | None = None
    """file_path'ы документов с флагом «целиком»: при попадании документа в
    результаты поиска в контекст включается его ПОЛНЫЙ текст (бюджет токенов
    расширяется под него). Ставится на сервере; не принимается от клиента."""
