...
const fetchGraph = async (label: string, maxDepth: number, maxNodes: number) => {
  ...
  try {
    ...
    useBackendState.getState().setErrorMessage(errorMessage(e), 'Ошибка запроса графа');
    return null;
  }
    ...
    if (!isFetching && !useGraphStore.getState().graphDataFetchAttempted) {
      ...
      dataPromise.then((result) => {
        ...
        if (result?.is_truncated) {
          toast.info(t('graphPanel.dataIsTruncated', 'Граф усечён до максимального числа узлов'));
        }
