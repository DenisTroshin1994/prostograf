...
import { adminGetDocMeta, adminSetDocMeta, type DocMeta } from '@/api/lightrag'
...
const metadataKeyLabels: Record<string, string> = {
  ...
  process_options: 'Параметры обработки',
  'process options': 'Параметры обработки',
  parser: 'Парсер',
  parse_engine: 'Движок парсинга',
  parse_format: 'Формат парсинга',
  parse_stage_skipped: 'Этап парсинга пропущен',
  analyzing_stage_skipped: 'Этап анализа пропущен',
  merge_stage_skipped: 'Этап слияния пропущен',
  parse_start_time: 'Начало парсинга',
  parse_end_time: 'Окончание парсинга',
  analyzing_start_time: 'Начало анализа',
  analyzing_end_time: 'Окончание анализа',
  process_start_time: 'Начало обработки',
  process_end_time: 'Окончание обработки',
  track_id: 'ID отслеживания',
  file_size: 'Размер файла',
  content_length: 'Длина содержимого',
  chunks_count: 'Количество фрагментов',
  is_duplicate: 'Является дубликатом',
  duplicate_kind: 'Тип дубликата',
  original_doc_id: 'Исходный doc_id',
  original_track_id: 'Исходный track_id'
}

const metadataKeyLabel = (key: string): string =>
  metadataKeyLabels[key] || metadataKeyLabels[key.replace(/ /g, '_')] || key
      ...
      return entries
        .map(([key, child]) => {
          ...
          if (typeof child === 'object' && child !== null && !Array.isArray(child)) {
            return `${prefix}${metadataKeyLabel(key)}:\n${renderedChild}`;
          }
          return `${prefix}${metadataKeyLabel(key)}: ${renderedChild}`;
        })
        .join('\n');
    }
  ...
  return Object.entries(formattedMetadata)
    ...
    .map(([key, value]) => {
      ...
      if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
        return `${metadataKeyLabel(key)}:\n${rendered}`;
      }
      return `${metadataKeyLabel(key)}: ${rendered}`;
    })
    .join('\n');
};
...
const formatDocumentDetails = (doc: DocStatusResponse): string => {
  ...
  if (doc.track_id) {
    details.push(`ID отслеживания: ${doc.track_id}`)
  }
  ...
  if (doc.error_msg) {
    details.push(`Сообщение об ошибке:\n${doc.error_msg}`)
  }
...
// Ячейка управления контекстом документа: METAINFO (подмешивается в генератор)
// и флаг «целиком» (документ включается в ответ полностью). Сам документ не меняется.
function DocMetaCell({
  filePath,
  meta,
  onSaved,
}: {
  filePath: string
  meta?: DocMeta
  onSaved: (fp: string, m: DocMeta) => void
}) {
  const [open, setOpen] = useState(false)
  const [text, setText] = useState(meta?.metainfo || '')
  const [saving, setSaving] = useState(false)
  useEffect(() => {
    if (open) setText(meta?.metainfo || '')
  }, [open, meta?.metainfo])
  const full = !!meta?.full
  const hasMeta = !!(meta?.metainfo && meta.metainfo.trim())

  const toggleFull = async (checked: boolean) => {
    try {
      const r = await adminSetDocMeta(filePath, null, checked)
      onSaved(filePath, { metainfo: r.metainfo, full: r.full })
    } catch (e: any) {
      toast.error(e?.response?.data?.detail || 'Не удалось сохранить флаг «целиком»')
    }
  }
  const saveMeta = async () => {
    setSaving(true)
    try {
      const r = await adminSetDocMeta(filePath, text, null)
      onSaved(filePath, { metainfo: r.metainfo, full: r.full })
      setOpen(false)
      toast.success('METAINFO сохранён')
    } catch (e: any) {
      toast.error(e?.response?.data?.detail || 'Не удалось сохранить METAINFO')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="flex items-center gap-2 whitespace-nowrap">
      <label className="flex cursor-pointer items-center gap-1 text-xs" title="Включать документ в ответ целиком (для процедурных инструкций)">
        <Checkbox checked={full} onCheckedChange={(c) => toggleFull(c === true)} />
        целиком
      </label>
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogTrigger asChild>
          <Button variant={hasMeta ? 'default' : 'outline'} size="sm" className="h-7 px-2 text-xs" tooltip="METAINFO документа">
            META
          </Button>
        </DialogTrigger>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            <DialogTitle>METAINFO документа</DialogTitle>
            <DialogDescription className="break-words">{filePath}</DialogDescription>
          </DialogHeader>
          <p className="text-muted-foreground text-xs">
            Текст подмешивается в генератор, когда документ попадает в ответ. Сам документ не меняется.
          </p>
          <textarea
            value={text}
            onChange={(e) => setText(e.target.value)}
            placeholder="Например: «Документ актуален с 2025 года; цены указаны без НДС»"
            className="min-h-32 w-full resize-y rounded-md border border-input bg-background p-2 text-sm"
          />
          <div className="flex justify-end">
            <Button size="sm" onClick={saveMeta} disabled={saving}>Сохранить</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
...
export default function DocumentManager() {
  ...
  // METAINFO + флаг «целиком» по документам (ключ — file_path).
  const [docMeta, setDocMeta] = useState<Record<string, DocMeta>>({})
  useEffect(() => {
    adminGetDocMeta().then(setDocMeta).catch(() => {})
  }, [])
  const onDocMetaSaved = useCallback((fp: string, m: DocMeta) => {
    setDocMeta((prev) => ({ ...prev, [fp]: m }))
  }, [])
                    ...
                    <Table className="w-full">
                      <TableHeader className="sticky top-0 bg-background z-10 shadow-sm">
                        <TableRow className="border-b bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/75 shadow-[inset_0_-1px_0_rgba(0,0,0,0.1)]">
                          ...
                          <TableHead className="text-center">Контекст</TableHead>
                            ...
                            <TableCell>
                              {doc.file_path ? (
                                <DocMetaCell
                                  ...
                                  meta={docMeta[doc.file_path]}
                                  onSaved={onDocMetaSaved}
                                />
                              ) : (
