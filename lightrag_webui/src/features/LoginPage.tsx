...
const LoginPage = () => {
  ...
  const { login, isAuthenticated, setBranding, appName, loginDescription } = useAuthStore()
  ...
  useEffect(() => {
    ...
    const checkAuthConfig = async () => {
      ...
      try {
        ...
        // Брендинг (название/описание) — задаётся админом, отдаётся сервером.
        setBranding(status.app_name || null, status.login_description || null)
  ...
  }, [isAuthenticated, login, navigate, setBranding])
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setBranding(response.app_name || null, response.login_description || null)
      ...
      if (isGuestMode) {
        ...
        toast.info(response.message || t('login.authDisabled', 'Аутентификация отключена. Используется гостевой доступ.'))
      } else {
      ...
      // Показываем реальную причину с сервера (например, 429 «слишком много
      // попыток»), а не всегда «неверный логин/пароль».
      const detail = (error as any)?.response?.data?.detail
      toast.error(detail || t('login.errorInvalidCredentials'))
  ...
  return (
    <div className="flex h-screen w-screen items-center justify-center bg-gradient-to-br from-background to-[#1f1e1c]">
        ...
        <CardHeader className="flex items-center justify-center space-y-2 pb-8 pt-6">
          <div className="flex flex-col items-center space-y-4">
            ...
            <div className="text-center space-y-2">
              <h1 className="font-serif text-3xl font-bold tracking-tight">{appName || SiteInfo.name}</h1>
              <p className="text-muted-foreground text-sm">
                {loginDescription || t('login.description')}
              </p>
            </div>
          </div>
        </CardHeader>
