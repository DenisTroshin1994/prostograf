    ...
    try {
      if (entityType === 'node' && entityId && nodeId) {
        ...
        if (operationStatus === 'success') {
          if (operationSummary?.merged) {
            ...
            try {
              ...
              throw createErrorWithCause('Не удалось обновить узел в графе', error)
            }
          ...
          const mergeError = operationSummary?.merge_error || 'Неизвестная ошибка'
        ...
        try {
          ...
          throw createErrorWithCause('Не удалось обновить связь в графе', error)
        }
