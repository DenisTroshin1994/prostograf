...
export default function QuerySettings() {
  ...
  const rewriteEnabled = useSettingsStore((state) => state.rewriteEnabled)

  return (
    <Card className="flex shrink-0 flex-col w-full max-w-2xl">
      ...
      <CardContent className="m-0 flex grow flex-col p-0 text-xs">
        <div className="size-full">
          <div className="flex flex-col gap-2 px-2 pr-2 pb-2">
            ...
            <>
              ...
              <div className="flex items-center gap-1">
                ...
                <ResetButton
                  ...
                  title="Сбросить к значению по умолчанию (Mix)"
                />
              </div>
            </>
            ...
            {/* Память диалогов (MultiTurn): сколько пар вопрос-ответ помнить */}
            <>
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <label htmlFor="history_turns" className="ml-1 cursor-help">
                      {t('retrievePanel.querySettings.historyTurns', 'Память диалогов (пар)')}
                    </label>
                  </TooltipTrigger>
                  <TooltipContent side="left">
                    <p>{t('retrievePanel.querySettings.historyTurnsTooltip', 'Сколько последних пар «вопрос-ответ» передавать модели для контекста, если сообщение не первое. 0 — без памяти.')}</p>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
              <div className="flex items-center gap-1">
                <Input
                  ...
                  className="h-9 flex-1 pr-2 [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none [-moz-appearance:textfield]"
                />
                <ResetButton onClick={() => handleChange('history_turns', 3)} title="Сбросить (3)" />
              </div>
            </>

            {/* Реврайт follow-up вопроса для поиска */}
            <>
              <div className="flex items-center gap-2">
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <label htmlFor="rewrite_enabled" className="flex-1 ml-1 cursor-help">
                        {t('retrievePanel.querySettings.rewriteEnabled', 'Реврайт вопроса для поиска')}
                      </label>
                    </TooltipTrigger>
                    <TooltipContent side="left">
                      <p>{t('retrievePanel.querySettings.rewriteEnabledTooltip', 'Если включено и в диалоге уже есть сообщения, модель переписывает уточняющий вопрос («А ещё?») в самостоятельный поисковый запрос с учётом истории. Промпт реврайта — на подвкладке «Промпты».')}</p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
                <Checkbox
                  ...
                  onCheckedChange={(checked) => useSettingsStore.getState().setRewriteEnabled(!!checked)}
                />
              </div>
            </>
