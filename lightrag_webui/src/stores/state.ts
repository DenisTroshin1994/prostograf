...
interface AuthState {
  ...
  role: string | null; // 'admin' | 'user' (из JWT)
  department: string | null; // отдел пользователя ('all' у admin)
  ...
  appName: string | null; // Кастомное название приложения (брендинг)
  loginDescription: string | null; // Кастомное описание на странице входа
  ...
  setBranding: (appName: string | null, loginDescription: string | null) => void;
...
const parseTokenPayload = (token: string): { sub?: string; role?: string; exp?: number; metadata?: Record<string, any> } => {
  try {
...
const getDepartmentFromToken = (token: string): string | null => {
  const payload = parseTokenPayload(token);
  const dept = payload.metadata?.department;
  if (typeof dept === 'string') return dept;
  return payload.role === 'admin' ? 'all' : '';
};
...
const initAuthState = (): { isAuthenticated: boolean; isGuestMode: boolean; role: string | null; department: string | null; coreVersion: string | null; apiVersion: string | null; username: string | null; webuiTitle: string | null; webuiDescription: string | null; appName: string | null; loginDescription: string | null; lastTokenRenewal: string | null; tokenExpiresAt: number | null } => {
  ...
  const appName = localStorage.getItem('LIGHTRAG-APP-NAME');
  const loginDescription = localStorage.getItem('LIGHTRAG-LOGIN-DESC');
...
export const useAuthStore = create<AuthState>(set => {
  ...
  return {
    ...
    loginDescription: initialState.loginDescription,
    ...
    setBranding: (appName, loginDescription) => {
      if (appName) localStorage.setItem('LIGHTRAG-APP-NAME', appName);
      else localStorage.removeItem('LIGHTRAG-APP-NAME');
      if (loginDescription) localStorage.setItem('LIGHTRAG-LOGIN-DESC', loginDescription);
      else localStorage.removeItem('LIGHTRAG-LOGIN-DESC');
      set({ appName: appName, loginDescription: loginDescription });
    },
